...
func resourceCosmicSecurityGroupRule() *schema.Resource {
	return &schema.Resource{
		Create: resourceCosmicSecurityGroupRuleCreate,
		Read:   resourceCosmicSecurityGroupRuleRead,
		Update: resourceCosmicSecurityGroupRuleUpdate,
		Delete: resourceCosmicSecurityGroupRuleDelete,
...
func resourceCosmicSecurityGroupRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if nrs := d.Get("rule").(*schema.Set); nrs.Len() > 0 {
		...
		rules := resourceCosmicSecurityGroupRule().Schema["rule"].ZeroValue().(*schema.Set)
	...
	return resourceCosmicSecurityGroupRuleRead(d, meta)
}
...
func createIngressOrEgressRule(cs *cosmic.CosmicClient, p authorizeSecurityGroupParams) (string, error) {
	switch p := p.(type) {
	case *cosmic.AuthorizeSecurityGroupIngressParams:
		r, err := cs.SecurityGroup.AuthorizeSecurityGroupIngress(p)
		if err != nil {
			return "", err
		}
		return r.Ruleid, nil
	case *cosmic.AuthorizeSecurityGroupEgressParams:
...
func resourceCosmicSecurityGroupRuleRead(d *schema.ResourceData, meta interface{}) error {
	cs := meta.(*cosmic.CosmicClient)
...
func readSecurityGroupRule(sg *cosmic.SecurityGroup, ruleIndex map[string]int, rule map[string]interface{}, uuid string) {
	uuids := rule["uuids"].(map[string]interface{})
	sgRules := append(sg.Ingressrule, sg.Egressrule...)
...
func resourceCosmicSecurityGroupRuleUpdate(d *schema.ResourceData, meta interface{}) error {
...
func resourceCosmicSecurityGroupRuleDelete(d *schema.ResourceData, meta interface{}) error {
