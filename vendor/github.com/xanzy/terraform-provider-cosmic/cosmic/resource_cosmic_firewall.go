...
func resourceCosmicFirewall() *schema.Resource {
	return &schema.Resource{
		Create: resourceCosmicFirewallCreate,
...
func resourceCosmicFirewallCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if nrs := d.Get("rule").(*schema.Set); nrs.Len() > 0 {
		...
		rules := resourceCosmicFirewall().Schema["rule"].ZeroValue().(*schema.Set)
	...
	return resourceCosmicFirewallRead(d, meta)
}
...
func resourceCosmicFirewallRead(d *schema.ResourceData, meta interface{}) error {
	cs := meta.(*cosmic.CosmicClient)
	...
	ruleMap := make(map[string]*cosmic.FirewallRule, l.Count)
...
func resourceCosmicFirewallUpdate(d *schema.ResourceData, meta interface{}) error {
	// Make sure all required parameters are there
	if err := verifyFirewallParams(d); err != nil {
		return err
	}
...
func resourceCosmicFirewallDelete(d *schema.ResourceData, meta interface{}) error {
